...
This package provides light and dark GNU Emacs themes whose theme-owned palettes and face mappings are calibrated above WCAG AA contrast minima. It does not claim that every Emacs face, external color, font, display, or overlap constitutes complete WCAG conformance. `complementary-light` uses a pure white background; `complementary-dark` uses a near-black neutral background. Both use gray neutrals plus exactly two registered accent families: primary and secondary. Most ordinary code remains neutral. Declarations and reserved syntax use the primary accent, while names, types, strings, and related constructs use the secondary accent. The themes change colors only, and only where the built-in `defface` already uses the corresponding color attribute. Bold, italic, underline, box, strike-through, inverse-video, height, width, inheritance, and extension behavior remain identical to the default `defface` specifications.
...
All theme-owned color literals are centralized in [complementary-light-palette.el](lisp/complementary-light-palette.el) and [complementary-dark-palette.el](lisp/complementary-dark-palette.el). Face declarations refer only to semantic tokens such as `background`, `foreground-muted`, `cursor`, `primary-text`, and `secondary-subtle`. Every accent palette provides `text`, `strong`, `on-strong`, `medium`, `on-medium`, `subtle`, `on-subtle`, `border`, `focus`, and `distant-foreground` tokens. Adaptive `state` and `on-state` tokens select a dark strong surface in the light theme and an intermediate accent border surface in the dark theme. The dark state surface is deliberately constrained between its near-black base and white state text, so the single white Emacs cursor also remains visible. The cursor uses a polarity-safe neutral—black in the light theme and white in the dark theme—instead of consuming an accent role.
...
The reviewed Emacs 30.2/GNU/Linux baseline is [emacs-30.el](inventory/emacs-30.el), while [current-generated.el](inventory/current-generated.el) contains the most recent generated inventory. The current baseline contains 1,171 faces: 166 `themed`, 526 `inherit`, 12 `alias`, 428 `preserve`, 39 `external-semantic`, 0 `excluded`, and 0 unclassified. The 166 themed faces are maintained as searchable, face-specific declarations rather than being painted by a single blanket rule. The expanded mappings include independently colored Org agenda and habit states, Isearch groups, Pulse, Smerge, ERC current nick, Elisp shorthands, and Newsticker secondary text.
...
- `reports/effective-face-contrast.json`: statically selected true-color foreground/background pairs after resolving theme rules, aliases, and literal inheritance; normal text and non-text indicators receive separate review gates, while hidden, decorative, inactive, contextual, and externally controlled faces are reported without misleading pass/fail claims; themed-face worst cases remain gated at the 5.0:1 text and 3.25:1 non-text design targets
...
- The color-vision report uses Machado, Oliveira, and Fernandes matrices at severities 0.1 through 1.0, a linear-sRGB grayscale transform, and [CIEDE2000](https://doi.org/10.1002/col.1049). CIEDE2000 is primarily validated for small adjacent color differences, and no universal Delta-E value establishes UI accessibility. The report is therefore diagnostic, deliberately has no pass/fail threshold, and describes the preset only as the best pair within the selected model, roles, and severity sweep.
