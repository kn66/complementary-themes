...
The color-vision preset selects `yellow` plus `blue`, the strongest worst-case pair within the generated Machado/CIEDE2000 model, its five semantic roles, and its severity sweep across all 66 distinct accent pairs and both themes. This is a model-relative ranking, not a universal perceptual threshold or accessibility guarantee. Grayscale separation is reported separately; state meaning continues to rely on the original face shape and text, not color alone.
...
- `reports/color-vision.json`: diagnostic CIEDE2000 distances under protanomaly, deuteranomaly, tritanomaly, and grayscale simulation, plus all-pair rankings and the `yellow`/`blue` preset; all 20,460 measurements are evaluated and the worst severity for each context is retained in the file, with explicit model-scope and metric-limit metadata
- `reports/effective-face-contrast.json`: statically selected true-color foreground/background pairs after resolving theme rules, aliases, and literal inheritance; the full inventory retains separate WCAG review observations, while themed-face worst cases use the light theme's low-emphasis accent targets and the dark theme's original stronger targets
