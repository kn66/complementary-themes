...
(defconst complementary-light--declared-themed-rules
  '(
    ...
    (font-lock-comment-delimiter-face :status themed :foreground comment-foreground)
    (font-lock-comment-face :status themed :foreground comment-foreground)
    ...
    (org-agenda-restriction-lock :status themed :background surface-sunken)
    (org-agenda-structure :status themed :foreground secondary-text)
    (org-clock-overlay :status themed :foreground foreground :background selection-neutral)
    (org-column :status themed :background surface-raised)
    (org-column-title :status themed :background surface-raised)
    ...
    (org-date-selected :status themed :foreground primary-text)
    (org-dispatcher-highlight :status themed :foreground primary-on-state :background primary-state)
    (org-drawer :status themed :foreground secondary-text)
    (org-latex-and-related :status themed :foreground secondary-text)
    (org-mode-line-clock-overrun :status themed :background primary-medium)
    (org-sexp-date :status themed :foreground secondary-text)
    (org-table :status themed :foreground secondary-text)
    (org-table-header :status themed :foreground foreground :background surface-raised)
...
Unsupported FORM values become nil instead of running arbitrary source code."
  (cond
   ...
   ((eq (car-safe form) 'assq)
    (let ((key (complementary-light--safe-template-value (nth 1 form)))
          (alist (complementary-light--safe-template-value (nth 2 form))))
      (when (listp alist)
        (copy-tree (assq key alist)))))
   ((eq (car-safe form) 'cond)
    (let ((clauses (cdr form)) matched value)
      (while (and clauses (not matched))
        (let ((clause (pop clauses)))
          (when (complementary-light--safe-template-value (car clause))
            (setq matched t)
            (dolist (expression (cdr clause))
              (setq value
                    (complementary-light--safe-template-value expression))))))
      value))
   (t nil)))
...
(defun complementary-light--unquote (object)
  ...
  (cond ((and (consp object) (eq (car object) 'quote)) (cadr object))
        ...
        ((and (consp object) (eq (car object) 'let))
         (let (symbols values)
           (dolist (binding (cadr object))
             (when (and (consp binding) (symbolp (car binding)))
               (push (car binding) symbols)
               (push (complementary-light--safe-template-value
                      (cadr binding))
                     values)))
           (cl-progv (nreverse symbols) (nreverse values)
             (let (value)
               (dolist (form (cddr object) value)
                 (setq value (complementary-light--unquote form)))))))
        (t object)))
